...
async function getStatus(marketplace: string): Promise<void> {
  try {
    ...
    const enabled = response.data['listing-approval-enabled'];
    if (enabled) {
...
async function enableApprovals(marketplace: string): Promise<void> {
  try {
    await apiPostTransit('/listing-approval/enable', { marketplace }, {});
...
async function disableApprovals(marketplace: string): Promise<void> {
  try {
    await apiPostTransit('/listing-approval/disable', { marketplace }, {});
